...
class AlignmentData:
    ...
    def __init__(self, data_dir="Mydata/Pre_data", rate=0.3, share=False, swap=False, val=0.0, with_r=False):
        t_ = time.time()
        file_num=[1,3]
        self.rate = rate
        self.val = val
        self.ins2id_dict, self.id2ins_dict, [self.kg1_ins_ids, self.kg2_ins_ids] = self.load_dict(data_dir + "/ent_ids_", file_num=file_num)
        self.rel2id_dict, self.id2rel_dict, [self.kg1_rel_ids, self.kg2_rel_ids] = self.load_dict(data_dir + "/rel_ids_", file_num=file_num)
    ...
    def load_label(self, data_dir, file_num):
        ...
        file_names = [data_dir+"_"+str(file_num[-1])+".txt"]
        ...
        for file_name in file_names:
            with open(file_name, "r", encoding="utf-8") as f:
                ...
                data = [tuple(map(int, i.split("\t")[0:2])) for i in data]
                triple += data
    ...
    def load_triples(self, data_dir, file_num):
        ...
        file_names = [data_dir + str(i)+".txt" for i in file_num]
